...
CGRect PTRectMakeWithAspectCenteredInRect(CGFloat aspect, CGRect rect){
	...
	const CGSize size = (sourceAspect > aspect) ?
	CGSizeMake(aspect*height, height) : CGSizeMake(width, width/aspect);
...
CGPoint PTRectGetOriginCenteredHorizontally(CGRect inner, CGRect outer) {
	...
	CGPoint origin = CGPointMake(PTRoundPixelValue(centerPoint.x - (inner.size.width / 2.0)),
								 inner.origin.y);
	return origin;
}
...
CGSize PTSizeThatFits(CGSize size, CGSize maxSize) {
	if (size.height > maxSize.height) {
		size = CGSizeMake(PTRoundPixelValue(size.width * maxSize.height / size.height), maxSize.height);
	}
	if (size.width > maxSize.width) {
		size = CGSizeMake(maxSize.width, PTRoundPixelValue(size.height * maxSize.width / size.width));
	}
	return size;
}
